...
package com.wipe.zc.journey.lib.materialspinner;
    ...
    public void setSelectedIndex(int position) {
        if (adapter != null) {
            if (position >= 0 && position <= adapter.getCount()) {
                ...
                if(onItemSelectedListener != null){
                    onItemSelectedListener.onItemSelected(MaterialSpinner.this, position, 0, null);
                }
            } else {
...
//        adapter = new MaterialSpinnerAdapter<>(getContext(), items).setTextColor(textColor);
        ...
        adapter = new MaterialSpinnerAdapter<>(getContext(), items, list_selected).setTextColor(textColor);
...
//      popupWindow.setOverlapAnchor(false);
     ...
     * 添加选中路径
     */
    public void addSelectedList(String dir){
        if(!list_selected.contains(dir)){
            list_selected.add(dir);
        }
    }
     ...
     * 移除选中路径
     */
    public void removeSelectedList(String dir){
        if(list_selected.contains(dir)){
            list_selected.remove(dir);
        }
    }
